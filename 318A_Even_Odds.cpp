...
int main()
{
    ...
    // If k is less than or equal to (n + 1) / 2, then the answer is 2 * k - 1
    // because in the first half, odd numbers are present and 2 * k - 1 gives the k-th odd number
    if (k <= (n + 1) / 2)
    {
        cout << 2 * k - 1 << endl;
    }
    ...
    {
        // Otherwise, the answer is 2 * (k - (n + 1) / 2)
        // because in the second half, even numbers are present, and we need to adjust the position
        cout << 2 * (k - (n + 1) / 2) << endl;
    }
    return 0;
